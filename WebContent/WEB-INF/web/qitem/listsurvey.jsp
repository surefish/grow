...
<body>
    ...
    <table class="tablelist">
        ...
        <tbody>
       		<s:iterator var="topic" value="listTopics.currentList">
			<tr>
				<td><input type="checkbox" name="" value=""></td>
				<td><s:property value="#topic.ftitle"/></td>
				<td>
				<s:date name= "#topic.fcreateTime" format= "yyyy-MM-dd HH:mm:ss"></s:date>    
				</td>
				<td><p>${topic.qSurvey.fname}</p></td>
				<td>
				<a href="<%=basePath%>qTopic/goItem?topicID=<s:property value="#topic.fuID" />"  class="tablelink">所有选项</a> 
				<a href="<%=basePath%>qTopic/getTTopicID?topicID=<s:property value="#topic.fuID" />"  class="tablelink">修改</a> 
				<a href="<%=basePath%>qTopic/deleteTopic?topicID=<s:property value="#topic.fuID "/>" onclick="return confirm('你确定要删除')" class="tablelink">删除</a>
				</td>
			</tr>
			</s:iterator>       
        </tbody>
	...
	function gotoPage(){
		...
		if(!reg.test(maxPage)){
			...
			if(currentPageNO>maxPage){//共几页
				...
				window.location.href="<%=basePath%>qitem/listsurvey.action?currentPageNO="+currentPageNO;		
			}
		}
	}
</script> 
